...
//口座番号チェック
  ...
  if($card_num == 'NULL'){
    $cnt = $cnt + 1;
  }elseif(mb_strlen($card_num) >= 14 && mb_strlen($card_num) <= 16){
    $cnt2 = $cnt + 1;
  }else{
    array_push($err, 'カード番号を１４桁~１６桁の間で入力してください。');
    }
  if($card_name == 'NULL'){
    $cnt = $cnt + 1;
  }elseif(mb_strlen($card_name) >=5 && mb_strlen($card_name) <= 25){
    $cnt2 = $cnt + 1;
  }else{
  array_push($err, 'カード名義を５桁~２５桁の間で入力してください。');
  }
  if($card_date == 'NULL'){
    $cnt = $cnt + 1;
  }elseif(mb_strlen($card_date) == 4){
    $cnt2 = $cnt + 1;
  }
  if(($cnt == 0 || $cnt == 3) || ($cnt2 == 0 || $cnt2 == 3)){
    $clear_card_num = $card_num;
    $clear_card_name = $card_name;
    $clear_card_date = $card_date;
  }elseif(($cnt == 1 || $cnt == 2) || ($cnt2 == 1 || $cnt2 == 2)){
    array_push($err, 'カード番号、カード名義、有効期限を正しく入力してください。');
  }
}
...
if(isset($_POST['regist']) && !isset($err[0])){
  ...
  $_SESSION['card_num'] = $clear_card_num;
  $_SESSION['card_name'] = $clear_card_name;
  $_SESSION['card_date'] = $clear_card_date;
